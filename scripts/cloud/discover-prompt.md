...
- **Check they are still live before they enter the feed.** On 2026-09-20, 23 of 41 sweep rows turned out to be dead by extraction time, which wasted the whole extract budget. Write the candidate URLs one per line to `/tmp/sweep-urls.txt` and run `node check-liveness.mjs --file /tmp/sweep-urls.txt` (Playwright, zero tokens). Drop every URL it reports expired; keep the uncertain ones. Report how many the check removed.
...
1. EXTRACT: every row of data/shortlist.tsv without a JD on disk -> jds/, then data/extracted.tsv (with track and visa_gate columns).
   **When WebFetch is refused or returns a bot-challenge page (403 / Cloudflare, e.g. instahyre.com, naukri.com), do not give up on the row:** retry it with the headless browser, `node browser-extract.mjs <url> --mode jd` (Playwright is installed in setup, zero tokens). Only mark a row unconfirmed if the browser fails too, and say which host blocked it.
