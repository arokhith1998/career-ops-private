...
import { compileTracks, resolveTrack, inTrackGeo } from './nightly-shortlist.mjs';
...
// ── Track gate ──────────────────────────────────────────────────────
// Optional, ADDED 2026-09-18. When config/nightly.yml defines `tracks`
// (different titles for different geographies), a job must match a title of
// the track that owns its region AND that track's metro list. The flat
// title_filter and location_filter cannot express "these titles only here".
// No tracks key, no file, or an unreadable file: every job passes.
export function buildTrackGate(nightlyPath = 'config/nightly.yml') {
  let tracks = null;
  try {
    if (existsSync(nightlyPath)) tracks = compileTracks(parseYaml(readFileSync(nightlyPath, 'utf-8')) || {});
  } catch { tracks = null; }
  if (!tracks) return null;
  return (job) => {
    const row = { title: job.title || '', location: job.location || '', url: job.url || '' };
    const res = resolveTrack(row, tracks);
    return !res.drop && inTrackGeo(row, res.track);
  };
}
...
async function main() {
  ...
  const trackGate = buildTrackGate();
  ...
  if (trackGate) {
    console.log(`Filtered by track:     ${totalFilteredTrack} removed (title not targeted in that geography)`);
  }
